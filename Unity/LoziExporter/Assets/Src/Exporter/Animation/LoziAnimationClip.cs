...
namespace Lozi
{
	public class LoziAnimationClip : HierarchyManager<HierarchyObject>
	{
		...
		public void generate()
		{
            ...
            EditorCurveBinding[] curvesArray = AnimationUtility.GetCurveBindings(animClip);
		...
		private void addToKeys(EditorCurveBinding curveData)
		{
            if(curveData.path.Length>0)
            {
                string prop = curveData.propertyName.Split('.')[0];
                string vec  = curveData.propertyName.Split('.')[1];

                AnimationCurve curve = AnimationUtility.GetEditorCurve(animClip, curveData);
                LoziAnimationKeyPoints point = getKeyPoint(curveData.path);

                switch (prop)
                {
                    case "m_LocalPosition": { parsePoint(vec, point.pos, curve.keys); break; }
                    case "m_LocalRotation": { parsePoint(vec, point.rot, curve.keys); break; }
                    case "m_LocalScale"   : { parsePoint(vec, point.scl, curve.keys); break; }
                }
            }
		}
